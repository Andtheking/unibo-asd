...
il numero di chiavi presenti nella tabella hash, senza doverle contare
di volta in volta. Tale informazione può essere mantenuta aggiornata
in tempo $O(1)$ durante l'esecuzione delle altre operazioni.
...
- Supponiamo di partire con una tabella hash di dimensione $k$, cioè una
  tabella hash in cui ci sono $k$ liste di collisione inizialmente
