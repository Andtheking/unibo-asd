...
static int min_child(const MinHeap *h, int i)
{
    ...
    if (valid(h, right) && h->heap[right].prio < h->heap[left].prio)
        min_child = right;
    else if (valid(h, left))
        min_child = left;
...
/* Cerca in tutto l'heap per trovare il nodo con chiave data
   ...
   Returns:
    ...
    - -1 Se non trova niente */
int search_key(MinHeap *h, int start, int key)
{
    int found = -1;
    int l_child = lchild(h, start);
    int r_child = rchild(h, start);

    /* Se l'elemento esiste, sei stato chiamato da una foglia */
    if (!valid(h, start))
        return -1;
    ...
    {
        found = search_key(h, l_child, key);
    }

    if (found == -1 && valid(h, r_child))
    {
        found = search_key(h, r_child, key);
    }
...
void minheap_change_prio(MinHeap *h, int key, double newprio)
{
    ...
    int parente = parent(h, element);

    if (valid(h, r_child) && h->heap[r_child].prio < h->heap[element].prio)
        move_down(h, element);
    else if (valid(h, l_child) && h->heap[l_child].prio < h->heap[element].prio)
        move_down(h, element);
